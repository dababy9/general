...
public class Part2 {
    ...
    public static void main(String[] args){
        try {
            ...
            int[][] p = new int[lines.size()][5];
            int index = 0;
            for(String[] line : lines)
                p[index++] = new int[]{getNum(line[2]), getNum(line[4]), getNum(line[6]), getNum(line[8]), getNum(line[10])};
            int best = Integer.MIN_VALUE;
            for(int a = 0; a <= 100; a++)
                for(int b = 0; b <= 100-a; b++)
                    for(int c = 0; c <= 100-a-b; c++){
                        int[] t = new int[5], amounts = new int[]{a, b, c, 100-a-b-c};
